...
$(document).ready(function() {
  $('#header').parent().prepend('<div id=\"Griffith logo\"><img src=\"figs/GRIFF_INT1_STD_RGB.png\" style=\"position:absolute; top:0; right:0; padding:20px; height:120px\"></div>');
...
Using Binder is as simple as clicking on the Binder badge - [![Launch Rstudio Binder](http://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/IdoBar/6003ESC_Workshop1/main?urlpath=rstudio).  
Alternatively, you can navigate to the [Binder](https://mybinder.org/) homepage and enter the URL of this tutorial [GitHub repository](https://github.com/IdoBar/6003ESC_Workshop1.git) `https://github.com/IdoBar/6003ESC_Workshop1.git` and click on the **launch** buton (see screenshot in `r figs(name="binder",display="cite")` below).
...
You should now see in your web browser an RStudio interface (if you got to the Jupyter page, click on new --> RStudio) and are should be ready to start working in R in "The Cloud"! `r  fa("cloud", , fill ="#5599FF")``r  fa("cloud", , fill ="#5599FF")``r  fa("cloud", , fill ="#5599FF")`

### Installing R and RStudio locally
Alternatively, both R and RStudio can be installed locally on any operating system (`r  fa("apple")`, `r  fa("windows",  fill ="#5599FF")`, or `r  fa("linux", )`, see a [detailed tutorial](https://www.datacamp.com/community/tutorials/installing-R-windows-mac-ubuntu%20)), which provides complete control over the installation, added packages and can be used anywhere without requiring internet connection. This is recommended for anyone who is planning to do any future serious analysis in R (including the assignments in this course).
...
### Install Packages
R can be extended with additional functionality by installing external packages (usually hosted at the Comprehensive R Archive Network repository -- [CRAN](https://cran.r-project.org/web/packages/index.html)). To find which packages can be useful for your type analysis, use search engine (Google is your friend) and the available [Task Views on CRAN](https://cran.r-project.org/web/views/), which provide some guidance which packages on CRAN are relevant for tasks related to a certain topic.  
For our current analysis we will use some packages from the [tidyverse](https://www.tidyverse.org/) -- a suite of packages designed to assist in data analysis, from reading data from multiple source (`readr`, `readxl` packages), through data wrangling and cleanup (such as `dplyr`, `tidyr`) and finally visualisation (`ggplot2`), as can be seen in `r figs(name="tidyverse_workflow", display="cite")`.  
_if tidyverse is failing to install then try [bplyr](https://github.com/yonicd/bplyr)_
...
There are lots of online tutorials on the use of `ggplot2`, one of my favourites is the [Beautiful plotting in R: A ggplot2 cheatsheet](http://zevross.com/blog/2014/08/04/beautiful-plotting-in-r-a-ggplot2-cheatsheet-3/).  
...
* R for Reproducible Scientific Analysis: A Software Carpentry course -- https://swcarpentry.github.io/r-novice-gapminder/ (offered regularly here at GU, follow [\@hackyhourGU](https://twitter.com/hackyhourGU) on Twitter)  
* From Data to Viz leads you to the most appropriate graph for your data -- https://www.data-to-viz.com/  
* [Coding Club](https://ourcodingclub.github.io/): A Positive Peer-Learning Community (ecology and environmental science students and researchers from the University of Edinburgh) -- https://ourcodingclub.github.io/  
* Beautiful plotting in R: A ggplot2 cheatsheet -- provides lots of recipes and easy fixes for common tasks when creating `ggplot2` plots [link](http://zevross.com/blog/2014/08/04/beautiful-plotting-in-r-a-ggplot2-cheatsheet-3/)  
* Useful cheatsheets can be found on the [RStudio Cheatsheets website](https://rstudio.com/resources/cheatsheets/) (start with the most useful/basic ones relevant to this course - RStudio IDE, Data Import, Data Transformation and Data Visualization)  
* [paletteer](https://github.com/EmilHvitfeldt/paletteer) -- An R package that let's you use colour palette from a range of available packages (see examples of available palettes [here](https://github.com/EmilHvitfeldt/r-color-palettes) and you can even use [TV-themed palettes](https://github.com/Ryo-N7/tvthemes), such as the example below from GOT) 

```{r GOT_palette, echo=FALSE, fig.cap=figs(name="GOT_palette"), out.width = '80%'}
knitr::include_graphics("https://camo.githubusercontent.com/a9c578c48fca929f0547aafdc6e21034e2f1952f/68747470733a2f2f692e696d6775722e636f6d2f477543757a79722e706e67")
```



### Appendix - Running R and RStudio on EcoCloud
[EcoCloud](https://ecocloud.org.au/) platform that provides this service for all students and staff of participating Australian and NZ universities and government agencies (including Griffith University). A guide on using RStudio within EcoCloud is available [here](https://support.ecocloud.org.au/support/solutions/articles/6000200390-using-rstudio).

>1. Please navigate to [EcoCloud](https://ecocloud.org.au/) and follow the prompts to login using your Griffith credentials (AAF login)  
